...
The first problem is to minimise the portfolio risk $\sigma_p^2$ given a portfolio mean $\mu_p=\mu$.\\It can be shown that the partial derivative of $\sigma_p^2 = \sum_{j=1}^n\sum_{i=1}^n w_iw_j\sigma_{ij}=\mathbf{w}^\text{T}\mathbf{Cw}$, with respect to $w_k$ is
\[
